<template>
  <div class="rounded-lg shadow-xl w-full h-auto bg-neutral-100 dark:bg-slate-700">
    <div class="flex flex-col">
      <div class="flex shadow-md">
        <div v-for="(tab, index) in tabs" :key="index"
          ...
          <input type="radio" name="tabs" :id="tab.value" :value="tab.value" v-model="activeTab" class="hidden" />
          <label :for="tab.value" class="text-lg font-bold cursor-pointer p-3" :class="{
            'text-slate-900 dark:text-slate-100 border-b-2 border-slate-900 dark:border-white':
              activeTab === tab.value,
            'text-gray-500 dark:text-gray-400': activeTab !== tab.value,
          }">
            {{ tab.label }}
          </label>
        </div>
